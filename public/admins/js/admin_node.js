...
function initGroup(moduleId) {
    ...
    $.post(APP_CURRENT, {type: 'group', module_id: moduleId}, function (ret) {
        if (ret.code) {
...
function initNode(moduleId, groupId) {
    ...
    $.post(APP_CURRENT, {type: 'node', module_id: moduleId, group_id: groupId}, function (ret) {
        if (ret.code) {
            layer.alert(ret.msg, {icon: 2});
        } else {
        ...
        items: [
            {
                label: '添加子节点',
                action: function () {
                    layer_open('添加', APP_CONTROLLER + '/add?pid=' + $(self).attr('data-module-id'));
                }
            },
            {
                label: '编辑模块',
                action: function () {
                    layer_open('编辑', APP_CONTROLLER + '/edit?id=' + $(self).attr('data-module-id'));
                }
            },
            {
                label: '删除模块',
                action: function () {
                    del('', $(self).attr('data-module-id'), APP_CONTROLLER + '/delete', function (data) {
                        if (!data.code) {
                            $(self).remove();
                        }
                    });
...
$(document).on('click', '.J_add', function () {
    var id = $(this).attr('data-id')
        ...
        || '0';
    layer_open('添加', APP_CONTROLLER + '/add?pid=' + id);
    return false;
    ...
    var id = $(this).attr('data-id')
        ...
        || (zTree.getCheckedNodes()[0] ? zTree.getCheckedNodes()[0].id : undefined)
        || '0';
    layer_open('批量导入', APP_CONTROLLER + '/load?pid=' + id);
});
// 编辑节点
function onEditName(treeId, treeNode) {
    layer_open('编辑', APP_CONTROLLER + '/edit?id=' + treeNode.id);
    return false;
}
// 删除节点
function onRemove(treeId, treeNode) {
    del('', treeNode.id, APP_CONTROLLER + '/delete', function (data) {
        if (!data.code) {
